...
class SearchService implements IService {
	...
	public retrieve(userAgent: string, line?: string): any {
		//return this.processData(line);
	//	console.log(line);
		return (line!==undefined)? this.context.retrieve(userAgent, this.processData(line)) : this.context.retrieve(userAgent);
	}
	
	private processData(data: string): any{
		var test = data.toUpperCase();
		switch(test){
			case "TRANSCARIOCA":
